export default {
  common: {
    enabled: 'Enabled',
    active: 'Active',
    disabled: 'Disabled',
    status: 'Status',
    actions: 'Actions',
    edit: 'Edit',
    delete: 'Delete',
    search: 'Search',
    reset: 'Reset',
    refresh: 'Refresh',
    confirm: 'Confirm',
    cancel: 'Cancel',
    all: 'All',
    yes: 'Yes',
    no: 'No',
    save: 'Save',
    close: 'Close',
    copy: 'Copy',
    success: 'Success',
    fail: 'Fail',
    loading: 'Loading...',
    noData: 'No data',
    total: '{0} items in total',
    createSuccess: 'Created successfully',
    updateSuccess: 'Updated successfully',
    deleteSuccess: 'Deleted successfully',
    deleteFail: 'Delete failed',
    operationFail: 'Operation failed',
    copied: 'Copied',
    copyFail: 'Copy failed',
    unlimited: 'Unlimited',
  },
  menu: {
    dashboard: 'Dashboard',
    provider: 'Providers',
    model: 'Models',
    key: 'API Keys',
    authRoles: 'Roles',
    authOrganizations: 'Organizations',
    authMembers: 'Members',
    ops: 'Operations',
    requestLogs: 'Request Logs',
    requestDebug: 'Request Debugging',
    usageStatistics: 'Usage Statistics',
    errorRules: 'Error Rules',
    promptTemplates: 'Prompt Templates',
    settings: 'Settings',
    license: 'License',
    pats: 'Access Tokens',
    playground: 'Playground',
    mcp: 'MCP',
    mcpServers: 'MCP Servers',
    globalDashboard: 'Dashboard',
    globalOrganizations: 'Organizations',
    globalRoles: 'Roles',
    contentSafety: 'Content Safety',
    safetyRules: 'Guardrail Rules',
    safetyAlerts: 'Alert Logs',
  },
  tier: {
    community: 'Community',
    pro: 'Professional',
    enterprise: 'Enterprise',
  },
  login: {
    title: 'Login',
    subtitle: 'CrossLink Admin',
    username: 'Username',
    password: 'Password',
    usernamePlaceholder: 'Username',
    passwordPlaceholder: 'Password',
    usernameRequired: 'Please enter username',
    passwordRequired: 'Please enter password',
    loginSuccess: 'Login successful',
    loginFail: 'Login failed, please check your username and password',
    captchaTitle: 'Security Verification',
    captchaTip: 'Drag the slider to complete the puzzle',
    captchaRefresh: 'Refresh',
    captchaFailed: 'Verification failed, please try again',
    captchaSuccess: 'Verified',
    logout: 'Logout',
    logoutConfirm: 'Confirm Logout',
    logoutContent: 'Are you sure you want to log out?',
    logoutOk: 'Logout',
    loggedOut: 'Logged out',
    user: 'User',
    bannerTitle1: 'Enterprise LLM',
    bannerTitle2: 'API Gateway Platform',
    bannerDesc: 'Unified multi-LLM management, smart routing, cost optimization',
    forceChangeTitle: 'Change Password',
    forceChangeDesc: 'For security, please change your default password before continuing.',
    forceNewPassword: 'New Password',
    forceConfirmPassword: 'Confirm Password',
    forceNewPasswordPlaceholder: 'At least 8 characters',
    forceConfirmPasswordPlaceholder: 'Re-enter new password',
    forceNewPasswordRequired: 'Please enter new password',
    forceConfirmPasswordRequired: 'Please confirm password',
    forcePasswordMinLength: 'Password must be at least 8 characters',
    forcePasswordMismatch: 'Passwords do not match',
    forceChangeSuccess: 'Password changed successfully, redirecting...',
    forceChangeFail: 'Failed to change password',
    forceChangeCancel: 'Cancel',
  },
  dashboard: {
    welcomeBack: 'Welcome back, {name}',
    admin: 'Admin',
    dateTemplate: '{weekday}, {month}/{day}/{year}',
    weekDays: {
      0: 'Sunday',
      1: 'Monday',
      2: 'Tuesday',
      3: 'Wednesday',
      4: 'Thursday',
      5: 'Friday',
      6: 'Saturday',
    },
    totalRequests: 'Total Requests',
    tokenUsage: 'Token Usage',
    totalCost: 'Total Cost',
    avgLatency: 'Avg Latency',
    requestTrend: 'Request Trend',
    requestCount: 'Requests',
    tokenCount: 'Tokens',
    modelDistribution: 'Model Distribution',
    proportion: 'Proportion',
    systemVersion: 'System Version',
    licenseStatus: 'License Status',
    dbStatus: 'Database Status',
    valid: 'Valid',
    invalid: 'Invalid',
    normal: 'Normal',
    abnormal: 'Error',
    noData: 'No data',
    totalSessions: 'Sessions',
    firstTokenLatency: 'First Token',
    errorRate: 'Error Rate',
    activeKeys: 'Active Keys',
    memberCount: 'Members',
    keyCount: 'Total Keys',
    costPerRequest: 'Cost / Request',
    costPer1k: 'Cost / 1K Tokens',
    performanceTrend: 'Latency & First-Token',
    errorRateTrend: 'Error Rate Trend',
    topModels: 'Top Models',
    topKeys: 'Top API Keys',
    topTeams: 'Top Teams',
    topnName: 'Name',
    topnDrillHint: 'Click for details',
    topnRequests: 'Requests',
    topnTokens: 'Tokens',
    topnCost: 'Cost',
    upgradeTitle: 'Unlock deeper analytics',
    upgradeDesc:
      'Upgrade to Pro for time-to-first-token, error-rate trends, and top key/team breakdowns.',
    range: {
      today: 'Today',
      '7d': 'Last 7 days',
      '30d': 'Last 30 days',
      '90d': 'Last 90 days',
    },
    rangeLabel: 'Time range',
    vsLastPeriod: 'vs. previous period',
  },
  provider: {
    title: 'Providers',
    addProvider: 'Add Provider',
    editProvider: 'Edit Provider',
    providerStatus: 'Provider status',
    modelSearch: 'Search model name',
    noProviders: 'No providers',
    noProvidersGuide: 'Click "Add Provider" above to connect your first AI provider',
    totalProviders: '{0} providers',
    addModel: 'Add Model',
    probeConnectivity: 'Test Connection',
    deleteProvider: 'Delete Provider',
    apiUrl: 'API URL',
    connected: 'Connected',
    connectFail: 'Connection failed',
    connectTestFail: 'Connection test request failed',
    testSelectModel: 'Select Model to Test',
    testModelAuto: 'Auto Select',
    testModelAutoDesc: 'System picks a model automatically',
    testModelItem: '{0} ({1})',
    testRunning: 'Testing…',
    testSuccess: 'Connected, latency {0}ms',
    testFail: 'Connection failed: {0}',
    testNoModels: 'No models for this provider',
    cannotDelete: 'Cannot Delete',
    cannotDeleteContent: 'This provider still has {0} models. Please delete all models first.',
    confirmDeleteProvider: 'Are you sure you want to delete provider "{name}"?',
    confirmDeleteModel: 'Are you sure you want to delete model "{name}"?',
    loadDataFail: 'Failed to load data',
    fetchModelListFail: 'Failed to fetch model list',
    noModels: 'No models',
    connectedModels: '{0} models',
    name: 'Identifier',
    namePlaceholder: 'Unique identifier, e.g. openai-main',
    displayName: 'Display Name',
    displayNamePlaceholder: 'Provider display name',
    adapterType: 'Adapter Type',
    adapterTypePlaceholder: 'Select adapter type',
    baseUrl: 'Base URL',
    baseUrlPlaceholder: 'https://api.example.com/v1',
    apiKey: 'API Key',
    apiKeyEditPlaceholder: 'Leave empty to keep unchanged',
    apiKeyCreatePlaceholder: 'Enter API Key',
    extraConfig: 'Extra Config',
    extraConfigPlaceholder: 'JSON format',
    nameRequired: 'Please enter an identifier',
    displayNameRequired: 'Please enter a display name',
    adapterTypeRequired: 'Please select an adapter type',
    extraConfigInvalidJson: 'Extra config is not valid JSON',
    modelTableName: 'Model Name',
    modelTableCallName: 'Call Name',
    modelTableWeight: 'Weight',
    modelTablePriority: 'Priority',
    modelTableInputPrice: 'Input Price',
    modelTableOutputPrice: 'Output Price',
    modelTableRoutingStrategy: 'Routing Strategy',
    modelTableProvider: 'Provider Model',
  },
  model: {
    title: 'Models',
    selectProvider: 'Select provider',
    routingStrategy: 'Routing strategy',
    searchModelName: 'Search model name',
    newModel: 'New Model',
    editModel: 'Edit Model',
    totalRecords: '{0} records',
    modelTableName: 'Model Name',
    modelTableProvider: 'Provider',
    modelTableProviderModel: 'Provider Model',
    modelTableWeight: 'Weight',
    modelTablePriority: 'Priority',
    modelTableInputPrice: 'Input Price',
    modelTableOutputPrice: 'Output Price',
    modelTableRoutingStrategy: 'Routing Strategy',
    modelTableStatus: 'Status',
    providerRequired: 'Please select a provider',
    modelNameRequired: 'Please enter model name',
    providerModelRequired: 'Please enter provider model',
    providerLabel: 'Provider',
    modelNameLabel: 'Model Name',
    providerModelLabel: 'Provider Model',
    weightLabel: 'Weight',
    priorityLabel: 'Priority',
    maxContext: 'Max Context',
    maxContextPlaceholder: 'Enter max context length',
    inputPriceLabel: 'Input Price',
    outputPriceLabel: 'Output Price',
    currencyLabel: 'Currency',
    currencyPlaceholder: 'Select currency',
    routingStrategyLabel: 'Routing Strategy',
    routingStrategyPlaceholder: 'Select routing strategy',
    supportsResponsesLabel: 'Upstream natively supports /v1/responses',
    supportsResponsesHelper:
      'When enabled, /v1/responses requests are passed through to the upstream verbatim (e.g. OpenAI, Azure). When disabled, requests are translated to Chat Completions.',
    statusLabel: 'Status',
    statusPlaceholder: 'Select status',
    providerPlaceholder: 'Select provider',
    modelNamePlaceholder: 'Enter model name',
    providerModelPlaceholder: 'Enter provider model',
    fetchModelListFail: 'Failed to fetch model list',
    fetchProviderListFail: 'Failed to fetch provider list',
    confirmDeleteModel: 'Are you sure you want to delete model "{name}"?',
    createFail: 'Create failed',
  },
  key: {
    title: 'API Keys',
    statusPlaceholder: 'Status',
    searchName: 'Search key name',
    totalKeys: '{0} keys',
    createKey: 'Create Key',
    editKey: 'Edit Key',
    tableName: 'Name',
    tablePrefix: 'Key Prefix',
    tableStatus: 'Status',
    tableAllowedModels: 'Allowed Models',
    tableTpm: 'TPM',
    tableRpm: 'RPM',
    tableBudget: 'Budget',
    tableCreatedAt: 'Created At',
    tableLastUsed: 'Last Used',
    tableActions: 'Actions',
    allModels: 'All',
    neverUsed: 'Never used',
    regenerate: 'Regenerate',
    regenerateConfirm: 'Regenerating will invalidate the old key immediately. Continue?',
    deleteConfirm: 'Are you sure you want to delete this key?',
    nameLabel: 'Name',
    namePlaceholder: 'Key name',
    nameRequired: 'Please enter a key name',
    allowedModelsLabel: 'Allowed Models',
    allowedModelsPlaceholder: 'Type model name and press Enter, leave empty for all',
    allowedRoutesLabel: 'Allowed Routes',
    allowedRoutesPlaceholder: 'Select allowed routes',
    tpmLimitLabel: 'TPM Limit',
    rpmLimitLabel: 'RPM Limit',
    limitPlaceholder: '0 = Unlimited',
    budgetLimitLabel: 'Budget Limit',
    budgetPlaceholder: '0 = Unlimited',
    budgetPeriodLabel: 'Budget Period',
    budgetPeriodPlaceholder: 'Select period',
    periodDaily: 'Daily',
    periodWeekly: 'Weekly',
    periodMonthly: 'Monthly',
    budgetDisplay: '¥{amount} / {period}',
    tableCallCount: 'Call Limit',
    callLimitLabel: 'Max Calls',
    callPlaceholder: '0 = Unlimited',
    callPeriodLabel: 'Call Period',
    callPeriodPlaceholder: 'Select period',
    keyCreatedTitle: 'Key Created',
    keyCreatedWarning:
      "Please save this key immediately. You won't be able to see it again after closing.",
    copyAndClose: 'Copy & Close',
    keyUpdateSuccess: 'Key updated successfully',
    fetchKeyListFail: 'Failed to fetch key list',
    keyDeleted: 'Key deleted',
    regenerateFail: 'Regeneration failed',
    keyCopied: 'Key copied to clipboard',
    copyFailManual: 'Copy failed, please select and copy the key manually',
    confirmedCopied: 'I have copied the key',
    periodDay: 'Day',
    periodWeek: 'Week',
    periodMonth: 'Month',
    tableExpiresAt: 'Expires',
    expiresAtLabel: 'Expiration',
    expireNever: 'Never',
    expireDays: 'In {n} days',
    expireCustom: 'Custom',
    expired: 'Expired',
    expiresSoon: '{n}d left',
    permanent: 'Permanent',
    errorInvalidRequest: 'Invalid request, please check your input',
    errorInvalidId: 'Invalid ID',
    errorNotFound: 'Key not found',
    errorBudgetPeriod: 'Period must be daily / weekly / monthly',
    errorBudgetNegative: 'Budget must be greater than or equal to 0',
    errorPermissions: 'Insufficient permissions',
  },
  auth: {
    roles: {
      title: 'Role Management',
      createRole: 'Create Role',
      editRole: 'Edit Role',
      tableName: 'Name',
      tableDisplayName: 'Display Name',
      tableUserCount: 'Users',
      tablePermissionCount: 'Permissions',
      namePlaceholder: 'Unique identifier, e.g. viewer',
      displayNamePlaceholder: 'Role display name',
      nameRequired: 'Please enter an identifier',
      displayNameRequired: 'Please enter a display name',
      cannotDelete: 'Cannot Delete',
      cannotDeleteContent: 'This role still has {0} users. Please remove or reassign them first.',
      confirmDelete: 'Are you sure you want to delete role "{name}"?',
      deleteSuccess: 'Role deleted',
      fetchFail: 'Failed to fetch role list',
      selectedCount: '{0} selected',
      selectAll: 'Select All',
      deselectAll: 'Deselect All',
    },
    users: {
      createUser: 'Create User',
      editUser: 'Edit User',
      tableUsername: 'Username',
      tableDisplayName: 'Display Name',
      tableRole: 'Role',
      tableStatus: 'Status',
      tableCreatedAt: 'Created At',
      usernamePlaceholder: 'Login username',
      passwordPlaceholder: 'At least 8 characters',
      displayNamePlaceholder: 'User display name',
      usernameRequired: 'Please enter a username',
      passwordRequired: 'Please enter a password',
      displayNameRequired: 'Please enter a display name',
      roleRequired: 'Please select a role',
      passwordMinLength: 'Password must be at least 8 characters',
      resetPassword: 'Reset Password',
      resetPasswordTitle: 'Reset User Password',
      resetPasswordPlaceholder: 'Enter new password',
      resetPasswordSuccess: 'Password reset successfully',
      resetPasswordFail: 'Failed to reset password',
      confirmDelete: 'Are you sure you want to delete user "{name}"?',
    },
    members: {
      title: 'Members',
      allMembers: 'All Members',
      noTeam: 'Unassigned',
      createTeam: 'Create Team',
      editTeam: 'Edit Team',
      addUser: 'Add User',
      moveToTeam: 'Move to Team',
      removeFromTeam: 'Remove from Team',
      assignToTeam: 'Assign to Team',
      moveConfirm: 'Move {name} from {from} to {to}?',
      removeConfirm: 'Remove {name} from {team}?',
      assignConfirm: 'Assign {name} to {team}?',
      deleteTeamWithMembers:
        'This team still has members. Please remove all members before deleting.',
      memberCount: '{count} members',
      teamInfo: '{count} members · ¥{budget}/{period} · TPM {tpm} · RPM {rpm}',
      teamInfoNoBudget: '{count} members · TPM {tpm} · RPM {rpm}',
      tableTeam: 'Team',
      tableTeamRole: 'Team Role',
      searchTeam: 'Search teams',
      searchUser: 'Search users',
    },
    teams: {
      createTeam: 'Create Team',
      editTeam: 'Edit Team',
      tableName: 'Name',
      tableDisplayName: 'Display Name',
      tableDescription: 'Description',
      tableBudget: 'Budget',
      tableRateLimit: 'Rate Limit',
      tableStatus: 'Status',
      namePlaceholder: 'Unique identifier, e.g. backend-team',
      displayNamePlaceholder: 'Team display name',
      descriptionPlaceholder: 'Team description',
      nameRequired: 'Please enter an identifier',
      displayNameRequired: 'Please enter a display name',
      confirmDelete: 'Are you sure you want to delete team "{name}"?',
      deleteSuccess: 'Team deleted',
      roleLeader: 'Leader',
      roleMember: 'Member',
    },
    organizations: {
      title: 'Organization Management',
      createOrg: 'Create Organization',
      editOrg: 'Edit Organization',
      tableName: 'Name',
      tableDisplayName: 'Display Name',
      tableBudget: 'Budget Usage',
      tableRateLimit: 'Rate Limit',
      namePlaceholder: 'Unique identifier, e.g. main-org',
      displayNamePlaceholder: 'Organization display name',
      descriptionPlaceholder: 'Organization description',
      adminEmailPlaceholder: 'Admin email (optional)',
      nameRequired: 'Please enter an identifier',
      displayNameRequired: 'Please enter a display name',
      confirmDelete: 'Are you sure you want to delete organization "{name}"?',
      deleteSuccess: 'Organization deleted',
      fetchFail: 'Failed to fetch organization list',
      detail: 'Details',
      tabMembers: 'Members',
      tabBudget: 'Budget',
      credentialsTitle: 'Organization Admin Credentials',
      credentialsWarning:
        'Please save the admin credentials immediately. The password cannot be viewed again after closing.',
      username: 'Username',
      password: 'Password',
      credentialsEmailSent: 'Admin credentials have been sent to {email}.',
      credentialsEmailFailed: 'Failed to send credentials to {email}. Please copy them manually.',
      copyAll: 'Copy All',
      copyAllSuccess: 'All credentials copied to clipboard',
      memberRoleOwner: 'Owner',
      memberRoleAdmin: 'Admin',
      memberRoleMember: 'Member',
      memberRoleViewer: 'Viewer',
      addMember: 'Add Member',
      selectUser: 'Select user',
      selectRole: 'Select role',
      removeMember: 'Remove',
      removeConfirm: 'Are you sure you want to remove this member?',
      fetchMembersFail: 'Failed to fetch member list',
      noMembers: 'No members',
      budgetLimit: 'Budget Limit',
      budgetPeriod: 'Budget Period',
      budgetSpent: 'Spent',
      budgetRemaining: 'Remaining',
      budgetUsage: 'Usage',
      calibrateBudget: 'Calibrate Budget',
      calibrateConfirm:
        'Are you sure you want to calibrate the budget? This will recalculate the spent amount.',
      calibrateSuccess: 'Budget calibrated successfully',
      calibrateFail: 'Failed to calibrate budget',
      fetchBudgetFail: 'Failed to fetch budget info',
      filterAll: 'All',
      filterActive: 'Active',
      filterDisabled: 'Disabled',
      cardMembers: 'Members',
      cardTeams: 'Teams',
      cardKeys: 'Keys',
      cardNoBudget: 'No budget limit',
      cardCreatedAt: 'Created',
    },
    permissions: {
      system: 'System',
      provider: 'Providers',
      model: 'Models',
      key: 'API Keys',
      user: 'User Management',
      team: 'Team Management',
      org: 'Organization Management',
      ops: 'Operations',
      security: 'Security',
      role: 'Role Management',
      mcp: 'MCP Services',
      errorRule: 'Error Rules',
      promptTemplate: 'Prompt Templates',
      routing: 'Routing',

      // System
      'system:password': 'Change Password',
      'system:view': 'View System',
      'system:update': 'Update System',
      'license:view': 'View License',
      'license:manage': 'Manage License',
      'playground:use': 'Use Playground',
      'insight:manage': 'Manage Insights',

      // Provider
      'provider:list': 'List Providers',
      'provider:create': 'Create Provider',
      'provider:update': 'Update Provider',
      'provider:delete': 'Delete Provider',
      'provider:test': 'Test Connectivity',

      // Model
      'model:list': 'List Models',
      'model:create': 'Create Model',
      'model:update': 'Update Model',
      'model:delete': 'Delete Model',

      // Key
      'key:list': 'List API Keys',
      'key:create': 'Create API Key',
      'key:update': 'Update API Key',
      'key:delete': 'Delete API Key',
      'key:regenerate': 'Regenerate Key',
      'key:rotate': 'Rotate Key',
      'key:hashes': 'View Key Hashes',

      // User
      'user:list': 'List Users',
      'user:create': 'Create User',
      'user:update': 'Update User',
      'user:delete': 'Delete User',

      // Team
      'team:list': 'List Teams',
      'team:create': 'Create Team',
      'team:update': 'Update Team',
      'team:delete': 'Delete Team',
      'team:manage_members': 'Manage Members',

      // Organization
      'org:list': 'List Organizations',
      'org:create': 'Create Organization',
      'org:update': 'Update Organization',
      'org:delete': 'Delete Organization',
      'org:manage_members': 'Manage Members',
      'org:view_billing': 'View Billing',
      'org:manage_billing': 'Manage Billing',

      // Operations
      'usage:list': 'View Usage',
      'usage:stats': 'Usage Statistics',
      'usage:export': 'Export Usage',
      'debug:list': 'View Debug Logs',
      'debug:clear': 'Clear Debug Logs',
      'audit:list': 'View Audit Logs',
      'audit:export': 'Export Audit Logs',
      'budget:manage': 'Manage Budget',

      // Security
      'guardrail:list': 'List Guardrails',
      'guardrail:create': 'Create Guardrail',
      'guardrail:update': 'Update Guardrail',
      'guardrail:delete': 'Delete Guardrail',
      'guardrail:test': 'Test Guardrail',
      'guardrail_alert:list': 'List Guardrail Alerts',
      'guardrail_alert:create': 'Create Guardrail Alert',
      'guardrail_alert:update': 'Update Guardrail Alert',
      'guardrail_alert:delete': 'Delete Guardrail Alert',
      'guardrail_alert:logs': 'View Guardrail Logs',
      'secret:test': 'Test Secret',
      'secret:manage': 'Manage Secrets',
      'agent_shield:view': 'View Agent Shield',
      'agent_shield:manage': 'Manage Agent Shield',
      'fingerprint:list': 'List Fingerprints',
      'fingerprint:view': 'View Fingerprint',
      'fingerprint:manage': 'Manage Fingerprints',

      // Role
      'role:list': 'List Roles',
      'role:create': 'Create Role',
      'role:update': 'Update Role',
      'role:delete': 'Delete Role',

      // MCP
      'mcp:list': 'List MCP Services',
      'mcp:view': 'View MCP Service',
      'mcp:create': 'Create MCP Service',
      'mcp:update': 'Update MCP Service',
      'mcp:delete': 'Delete MCP Service',
      'mcp:permission': 'Manage MCP Permissions',
      'mcp:logs': 'View MCP Logs',
      'mcp:stats': 'View MCP Statistics',

      // Error Rules
      'error_rule:list': 'List Error Rules',
      'error_rule:create': 'Create Error Rule',
      'error_rule:update': 'Update Error Rule',
      'error_rule:delete': 'Delete Error Rule',

      // Prompt Templates
      'template:list': 'View Templates',
      'template:create': 'Create Template',
      'template:update': 'Update Template',
      'template:delete': 'Delete Template',

      // Routing
      'routing:stats': 'View Routing Statistics',
    },
  },
  promptTemplate: {
    title: 'Prompt Templates',
    subtitle:
      'Manage reusable system prompt + few-shot context blocks referenced by the gateway via x_context',
    totalTemplates: '{0} templates',
    createTemplate: 'New Template',
    editTemplate: 'Edit Template',
    deleteConfirm: 'Delete this template?',
    nameConflict: 'Template name already exists',
    createSuccess: 'Template created',
    updateSuccess: 'Template updated',
    deleteSuccess: 'Template deleted',
    fetchFail: 'Failed to load templates',
    searchName: 'Search by name',
    filterFormat: 'Target format',
    filterStatus: 'Status',
    statusEnabled: 'Enabled',
    statusDisabled: 'Disabled',
    colName: 'Name',
    colFormat: 'Format',
    colDescription: 'Description',
    colUpdatedAt: 'Updated',
    preview: 'Preview',
    previewTitle: 'Preview - {name}',
    previewHint: 'Preview is rendered locally; no real request is sent',
    previewVariables: 'Variables',
    previewRun: 'Run Preview',
    previewResult: 'Rendered Result',
    copySystem: 'Copy System Prompt',
    formName: 'Name',
    formNamePh: 'Template name',
    formNameRequired: 'Name is required',
    formDescription: 'Description',
    formDescriptionPh: 'Template description',
    formSystemPrompt: 'System Prompt',
    formSystemPromptPh: 'Enter system prompt',
    formSystemPromptHint:
      'Use {{variable}} as placeholder; only trusted variables are interpolated',
    formVariables: 'Variables',
    formVariablesHint:
      'The backend does not validate variable names — keep them valid and unique. Reserved-name variables are forced untrusted.',
    formVarNamePh: 'Variable name',
    formVarDefaultPh: 'Default',
    formFewShot: 'Few-shot Examples',
    formFewShotHint: 'Static examples, no variable interpolation',
    formFewShotContentPh: 'Example content',
    required: 'Required',
    trusted: 'Trusted',
    addVariable: 'Add Variable',
    addFewShot: 'Add Example',
    warnReservedName:
      'Reserved-name variable — forced untrusted, cannot appear in the system prompt',
    warnUntrusted: 'Untrusted variable — cannot appear in the system prompt',
    errVarNameEmpty: 'Variable name is required',
    errVarNameFormat:
      'Variable name may only contain letters, digits and underscore, not starting with a digit',
    errVarNameDuplicate: 'Duplicate variable name',
    previewErrors: {
      missing_variable: 'A required variable was not provided',
      untrusted_var_in_system: 'Untrusted variable cannot appear in the system prompt',
      value_too_large: 'Variable value too large (>4KB)',
      system_too_large: 'Rendered system prompt exceeds 8KB',
      type_mismatch: 'Variable type mismatch',
      invalid_schema: 'Invalid variables schema',
      invalid_few_shot: 'Invalid few-shot config',
      fallback: 'Preview failed',
    },
  },
  errorRule: {
    title: 'Error Rules',
    subtitle:
      'Classify upstream errors (e.g. as quota-type persistent faults) to drive retry and fallback decisions',
    totalRules: '{0} rules',
    createRule: 'New Rule',
    editRule: 'Edit Rule',
    deleteConfirm: 'Delete this rule?',
    createSuccess: 'Rule created',
    updateSuccess: 'Rule updated',
    deleteSuccess: 'Rule deleted',
    fetchFail: 'Failed to load rules',
    global: 'Global',
    filterMatchField: 'Match field',
    filterScope: 'Scope',
    searchPattern: 'Search pattern',
    colMatchField: 'Match Field',
    colPattern: 'Pattern',
    colClassification: 'Classification',
    colProviderType: 'Provider Type',
    colScope: 'Scope',
    colPriority: 'Priority',
    formMatchField: 'Match field',
    formPattern: 'Pattern',
    formClassification: 'Classification',
    formProviderType: 'Provider type',
    formProviderTypePlaceholder: 'Leave empty = global (all providers)',
    formProviderTypeHint: 'Empty means the rule applies to all providers',
    formScope: 'Scope',
    formPriority: 'Priority',
    formPriorityHint: 'Lower number = higher priority',
    formMatchFieldRequired: 'Please select a match field',
    formPatternRequired: 'Please enter a pattern',
    patternPlaceholderMessage: 'Substring match, e.g. quota exceeded',
    patternPlaceholderExact: 'Exact match, e.g. 429',
    matchField: {
      status: 'HTTP Status',
      code: 'Error Code',
      type: 'Error Type',
      message: 'Error Message',
    },
    scope: {
      account: 'Account',
      model: 'Model',
    },
  },
  mcp: {
    title: 'MCP Servers',
    subtitle:
      'Manage MCP servers, discover tools, configure call permissions, and inspect logs & stats',
    totalServers: '{0} servers',
    createServer: 'New Server',
    editServer: 'Edit Server',
    deleteConfirm: 'Delete MCP server "{name}"?',
    deleteSuccess: 'Server deleted',
    fetchFail: 'Failed to load servers',
    testRunning: 'Testing…',
    test: 'Test',
    testSuccess: 'Healthy',
    testFail: 'Unhealthy: {0}',
    back: 'Back',
    colName: 'Name',
    colTransport: 'Transport',
    colHealth: 'Health',
    colToolCount: 'Tools',
    colEnabled: 'Enabled',
    colActions: 'Actions',
    healthHealthy: 'Healthy',
    healthUnknown: 'Unknown',
    healthUnhealthy: 'Unhealthy',
    formName: 'Identifier',
    formNamePlaceholder: 'Unique, 3-64 chars, lowercase alnum and - _',
    formDisplayName: 'Display Name',
    formDescription: 'Description',
    formTransport: 'Transport',
    formUrl: 'Endpoint URL',
    formUrlPlaceholder: 'https://mcp.example.com/mcp',
    formStdio: 'STDIO Config',
    formCommand: 'Command',
    formCommandPlaceholder: 'e.g. npx',
    formArgs: 'Args',
    formArgsPlaceholder: 'Press Enter to add',
    formEnv: 'Environment',
    formAuthType: 'Auth Type',
    formAuthConfig: 'Auth Config',
    formToken: 'Token',
    formUsername: 'Username',
    formPassword: 'Password',
    formClientId: 'Client ID',
    formClientSecret: 'Client Secret',
    formTokenUrl: 'Token URL',
    formScopes: 'Scopes',
    formAccessKey: 'Access Key',
    formSecretKey: 'Secret Key',
    formRegion: 'Region',
    formService: 'Service',
    formCustomHeaders: 'Custom Headers',
    formHeadersHint: 'Key-value pairs; leave empty when editing to keep existing',
    formSecretHint: 'Sensitive fields are not echoed on edit; leave empty to keep unchanged',
    formEnabled: 'Enabled',
    nameRequired: 'Please enter an identifier',
    transportRequired: 'Please select a transport type',
    tabTools: 'Tools',
    tabPermissions: 'Permissions',
    tabLogs: 'Logs',
    tabStats: 'Stats',
    toolsCount: '{0} tools registered',
    toolsEmpty: 'No tools yet — test the connection to discover tools',
    toolName: 'Tool',
    toolDescription: 'Description',
    permCreate: 'Add Permission',
    permPrincipalType: 'Principal Type',
    permPrincipal: 'Principal',
    permAllow: 'Allow Tools',
    permDeny: 'Deny Tools',
    permAllowPlaceholder: 'Tool name, Enter to add; * for all',
    permDenyPlaceholder: 'Tool name, Enter to add',
    permPrincipalKey: 'API Key',
    permPrincipalTeam: 'Team',
    permPrincipalRole: 'Role',
    permSelectPrincipal: 'Select principal',
    permDeleteConfirm: 'Delete this permission?',
    permEmpty: 'No permission rules; all callers can access every tool by default',
    logTool: 'Tool',
    logMethod: 'Method',
    logStatus: 'Status',
    logDuration: 'Duration',
    logInput: 'Input',
    logOutput: 'Output',
    logError: 'Error',
    logTime: 'Time',
    logStatusSuccess: 'Success',
    logStatusError: 'Error',
    logStatusBlocked: 'Blocked',
    logEmpty: 'No call logs',
    statsTotal: 'Total Calls',
    statsSuccess: 'Success',
    statsError: 'Errors',
    statsBlocked: 'Blocked',
    statsAvgDuration: 'Avg Duration',
    statsP95: 'P95 Duration',
    statsInputBytes: 'Total Input',
    statsOutputBytes: 'Total Output',
    statsTopTools: 'Top Tools',
    statsTrend: 'Call Trend',
    statsToolName: 'Tool',
    statsToolCount: 'Calls',
    statsToolAvgDuration: 'Avg Duration',
    statsToolErrorRate: 'Error Rate',
    statsDays: 'Last {0} days',
  },
  safety: {
    title: 'Content Safety',
    rules: {
      title: 'Guardrail Rules',
      createRule: 'Create Rule',
      editRule: 'Edit Rule',
      deleteRule: 'Delete Rule',
      deleteConfirm: 'Are you sure you want to delete this rule?',
      testName: 'Test Rule',
      testInput: 'Enter test text',
      testResult: 'Test Result',
      blocked: 'Blocked',
      passed: 'Passed',
      noResult: 'No results',
      formName: 'Rule Name',
      formType: 'Engine Type',
      formDirection: 'Direction',
      formSeverity: 'Severity',
      formAction: 'Action',
      formConfig: 'Engine Config',
      formModelFilter: 'Model Filter',
      formModelFilterPh: 'Select or type model names',
      formEnabled: 'Enabled',
      filterType: 'Engine Type',
      filterStatus: 'Status',
      searchName: 'Search rule name',
      configTitle: 'Global Config',
      configEnabled: 'Enable Guardrails',
      configLogOnly: 'Log Only Mode',
      configFailOpen: 'Fail Open',
      configEnabledTip: 'When enabled, all guardrail rules will apply to requests and responses',
      configLogOnlyTip: 'Only log violations without blocking or masking',
      configFailOpenTip: 'Allow requests through when guardrail engine fails, instead of rejecting',
    },
    alerts: {
      title: 'Alert Logs',
      detail: 'Alert Detail',
      overview: 'Overview',
      todayCount: 'Today',
      weekCount: 'This Week',
      criticalCount: 'Critical',
      filterEngine: 'Engine Type',
      filterSeverity: 'Severity',
      filterAction: 'Action',
      filterStatus: 'Status',
      filterTimeRange: 'Time Range',
      showContent: 'Show content preview',
      colTime: 'Time',
      colRule: 'Rule Name',
      colEngine: 'Engine',
      colSeverity: 'Severity',
      colAction: 'Action',
      colDirection: 'Direction',
      colModel: 'Model',
      colStatus: 'Status',
      reason: 'Reason',
      contentPreview: 'Content Preview',
      context: 'Context',
      notifyStatus: 'Notification Status',
      apiKeyId: 'API Key ID',
    },
    direction: {
      request: 'Request',
      response: 'Response',
      both: 'Both',
      ingress: 'Ingress',
      egress: 'Egress',
    },
    severity: {
      low: 'Low',
      medium: 'Medium',
      high: 'High',
      warning: 'Warning',
      critical: 'Critical',
    },
    action: { block: 'Block', log: 'Log', mask: 'Mask' },
    alertStatus: {
      logged: 'Logged',
      sent: 'Sent',
      partial: 'Partial',
      failed: 'Failed',
      dropped: 'Dropped',
      blocked: 'Blocked',
    },
    engineType: {
      keyword_filter: 'Keyword Filter',
      pii_detection: 'PII Detection',
      prompt_injection: 'Prompt Injection',
      credential_detection: 'Credential Detection',
      openai_moderation: 'OpenAI Moderation',
      content_length: 'Content Length',
      agent_fingerprint: 'Agent Fingerprint',
      behavior_analysis: 'Behavior Analysis',
      webhook: 'Webhook',
      ip_binding: 'IP Binding',
    },
    config: {
      modeForm: 'Form Mode',
      modeJson: 'JSON Mode',
      complexFieldHint: 'This engine has complex fields ({0}). Switch to JSON mode to edit them.',
      jsonInvalid: 'Invalid JSON format. Please fix before switching.',
      unknownEngine: 'This engine type does not support form editing. Please use JSON mode.',
      // keyword_filter
      keyword: {
        blocklist: 'Blocklist',
        blocklistPh: 'Press Enter to add keywords',
        allowlist: 'Allowlist',
        allowlistPh: 'Press Enter to add exempt keywords',
        useRegex: 'Use Regex',
        caseSensitive: 'Case Sensitive',
      },
      // content_length
      length: {
        maxInputChars: 'Max Input Characters',
        maxOutputChars: 'Max Output Characters',
      },
      // pii_detection
      pii: {
        types: 'PII Types',
        action: 'Detection Action',
        typeEmail: 'Email',
        typePhone: 'Phone',
        typeCnId: 'CN ID Card',
        typeCreditCard: 'Credit Card',
        typeIpAddress: 'IP Address',
      },
      // openai_moderation
      moderation: {
        apiKey: 'API Key',
        apiKeyPh: 'Enter OpenAI API Key',
        baseUrl: 'Base URL',
        baseUrlPh: 'https://api.openai.com',
        categories: 'Categories',
        catHate: 'Hate',
        catViolence: 'Violence',
        catSexual: 'Sexual',
        catHarassment: 'Harassment',
        threshold: 'Threshold',
        thresholdTip: '0~1, lower values are stricter',
        blockOnError: 'Block on Error',
        timeoutMs: 'Timeout (ms)',
      },
      // prompt_injection
      injection: {
        sensitivity: 'Sensitivity',
        sensLow: 'Low',
        sensMedium: 'Medium',
        sensHigh: 'High',
        indirectInjection: 'Detect Indirect Injection',
        indirectInjectionTip: 'Detect malicious instructions injected via external data',
        agentHijack: 'Detect Agent Hijack',
        configProbe: 'Detect Config Probe',
        decodeBypass: 'Detect Decode Bypass',
        threshold: 'Custom Threshold',
        thresholdPh: 'Leave empty for sensitivity-based default',
        thresholdTip: 'Override sensitivity threshold, lower values are stricter',
      },
      // credential_detection
      credential: {
        categories: 'Credential Categories',
        catApiKey: 'API Key',
        catCloud: 'Cloud Credential',
        catPrivateKey: 'Private Key',
        catDatabase: 'Database String',
        catJwt: 'JWT',
        catEnvFile: 'Env File',
        catCustom: 'Custom',
        customPatterns: 'Custom Patterns',
        customPatternsTip: 'Custom regex patterns — switch to JSON mode to edit',
        maskChar: 'Mask Character',
        maskKeepPrefix: 'Mask Keep Prefix',
        maskKeepSuffix: 'Mask Keep Suffix',
        maxMatches: 'Max Matches Per Check',
      },
      // webhook
      webhook: {
        url: 'Webhook URL',
        urlPh: 'https://example.com/webhook',
        headers: 'Custom Headers',
        headersTip: 'Custom HTTP headers — switch to JSON mode to edit',
        timeoutMs: 'Timeout (ms)',
        blockOnError: 'Block on Error',
      },
      // agent_fingerprint
      fingerprint: {
        mode: 'Mode',
        modeTag: 'Tag',
        modeBlock: 'Block',
        blockAgentTypes: 'Block Agent Types',
        blockAgentTypesPh: 'Press Enter to add agent type names',
        customPatterns: 'Custom Patterns',
        customPatternsTip: 'Custom agent fingerprint patterns — switch to JSON mode to edit',
        detectAutomated: 'Detect Automated Behavior',
        automationThresholdRpm: 'Automation Threshold (RPM)',
        automationWindow: 'Automation Window (seconds)',
      },
      // behavior_analysis
      behavior: {
        rules: 'Behavior Rules',
        rulesTip: 'Behavior rules are complex — switch to JSON mode to edit',
      },
    },
  },
  ops: {
    requestLogs: 'Request Logs',
    dateRange: 'Date Range',
    statusCode: 'Status Code',
    model: 'Model',
    advancedFilter: 'Advanced Filter',
    collapseFilter: 'Collapse Filter',
    provider: 'Provider',
    apiKey: 'API Key',
    fallback: 'Fallback',
    tableTime: 'Time',
    tableModel: 'Model',
    tableModelUsed: 'Actual Model',
    tableStatus: 'Status',
    tableInput: 'Input',
    tableOutput: 'Output',
    tableLatency: 'Latency',
    tableTtft: 'TTFT',
    tableCost: 'Cost',
    tableTags: 'Tags',
    noLogs: 'No request logs',
    fetchLogsFail: 'Failed to fetch request logs',
    tagFallback: 'Fallback',
    tagCache: 'Cache',
    tagGuardrail: 'Guardrail',
  },
  usageStat: {
    title: 'Usage Statistics',
    subtitle:
      'Aggregate analysis of requests, tokens and cost by time, model, provider, team and key',
    range: 'Time Range',
    team: 'Team',
    metric: 'Metric',
    metricRequests: 'Requests',
    metricTokens: 'Tokens',
    metricCost: 'Cost',
    metricInputTokens: 'Input Tokens',
    metricOutputTokens: 'Output Tokens',
    metricReasoningTokens: 'Reasoning Tokens',
    metricCacheRead: 'Cache Read Tokens',
    fallbackDaily: 'Fallback',
    retryDaily: 'Retry',
    guardrailDaily: 'Guardrail',
    kpiRequests: 'Total Requests',
    kpiTokens: 'Total Tokens',
    kpiCost: 'Total Cost',
    kpiLatency: 'Avg Latency',
    kpiFirstToken: 'Avg First Token',
    kpiErrorRate: 'Error Rate',
    kpiCostPer1k: 'Cost / 1K Tokens',
    kpiCostPerReq: 'Cost / Request',
    kpiFallbackRate: 'Fallback Rate',
    kpiRetryRate: 'Retry Rate',
    kpiGuardrailRate: 'Guardrail Rate',
    kpiActiveKeys: 'Active Keys',
    colModel: 'Model',
    colTeam: 'Team',
    colTemplate: 'Template',
    topTemplates: 'Top Templates',
    colRequests: 'Requests',
    colTokens: 'Tokens',
    colCost: 'Cost',
    colShare: 'Share',
    routingTitle: 'Provider Routing Distribution',
    routingModelPlaceholder: 'Select a model',
    routingPlaceholder:
      'Select a model to compare each provider’s configured weight vs actual traffic',
    routingForbidden: 'Your account has no permission to view routing statistics',
    colConfigWeight: 'Config Weight',
    colActual: 'Actual Share',
    colDeviation: 'Deviation',
    empty: 'No data for the current filters',
    fetchFail: 'Failed to fetch statistics',
  },
  requestDebug: {
    title: 'Request Debugging',
    subtitle:
      'Capture and inspect the full request/response and upstream chain of each gateway call (in-memory, max 100 entries, cleared on restart)',
    debugMode: 'Debug Mode',
    debugModeOn: 'Enabled',
    debugModeOff: 'Disabled',
    debugModeHint: 'Debug mode can be toggled in System Settings (Pro)',
    autoRefresh: 'Auto refresh',
    clear: 'Clear',
    clearConfirm: 'Clear all captured debug entries?',
    clearSuccess: 'Cleared',
    clearFail: 'Failed to clear',
    fetchFail: 'Failed to fetch debug entries',
    empty: 'No debug entries',
    emptyHint: 'Enable debug mode and make a call — entries will appear here',
    filters: 'Filters',
    status: 'Status',
    statusAll: 'All',
    statusSuccess: 'Success',
    statusFail: 'Failed',
    path: 'Path',
    keyword: 'Search (model / path / ID)',
    colTime: 'Time',
    colModel: 'Model',
    colPath: 'Path',
    colStatus: 'Status',
    colDuration: 'Duration',
    colStream: 'Stream',
    colUpstream: 'Upstream calls',
    stream: 'Stream',
    nonStream: 'Non-stream',
    retry: 'Retry',
    fallback: 'Fallback',
    yes: 'Yes',
    no: 'No',
    back: 'Back to list',
    tabRaw: 'Raw',
    tabUpstream: 'Upstream chain',
    tabAnalysis: 'Conversation',
    basicInfo: 'Basic info',
    reqHeaders: 'Request headers',
    reqBody: 'Request body',
    respHeaders: 'Response headers',
    respBody: 'Response body',
    respStatus: 'Response status',
    truncated: 'Content truncated',
    noUpstream: 'No upstream calls recorded',
    upstreamStep: 'Hop {0}',
    attempt: 'Attempt {0}',
    error: 'Error',
    analysisFail: 'Analysis failed — check API or license',
    analysisLoading: 'Parsing conversation…',
    timeline: 'Conversation timeline',
    parseError: 'Parse warning',
    noTimeline: 'No conversation content',
    usage: 'Usage',
    inputTokens: 'Input tokens',
    outputTokens: 'Output tokens',
    reasoningTokens: 'Reasoning tokens',
    estCost: 'Estimated cost',
    stopReason: 'Stop reason',
    tools: 'Tool calls',
    toolSummary: 'Tool summary',
    toolName: 'Tool',
    toolCount: 'Calls',
    toolSuccess: 'Success',
    toolFailed: 'Failed',
    toolInput: 'Input',
    toolOutput: 'Output',
    roleUser: 'User',
    roleAssistant: 'Assistant',
    roleTool: 'Tool',
    typeText: 'Text',
    typeThinking: 'Thinking',
    typeToolUse: 'Tool call',
    typeToolResult: 'Tool result',
    typeImage: 'Image',
    proOnlyHint: 'Conversation analysis is a Pro feature',
  },
  logDetail: {
    basicInfo: 'Basic Info',
    routeType: 'Route Type',
    errorType: 'Error Type',
    agentType: 'Agent Type',
    usageStats: 'Usage Stats',
    totalTokens: 'Total Tokens',
    costWithCurrency: 'Cost ({currency})',
    input: 'Input',
    output: 'Output',
    modelAndRouting: 'Model & Routing',
    requestModel: 'Requested Model',
    actualModel: 'Actual Model',
    modelFallback: 'Model fallback occurred',
    provider: 'Provider',
    apiKey: 'API Key',
    team: 'Team',
    performanceMetrics: 'Performance',
    totalLatency: 'Total Latency',
    firstToken: 'First Token',
    faultTolerance: 'Fault Tolerance & Cache',
    fallbackCount: 'Fallback Count',
    retryCount: 'Retry Count',
    cacheHit: 'Cache Hit',
    security: 'Security',
    guardrailTriggered: 'Guardrail Triggered',
    guardrailRule: 'Guardrail Rule',
    securityEvents: 'Security Events',
    requestResponseContent: 'Request / Response',
    requestContent: 'Request Content',
    responseContent: 'Response (may be incomplete)',
    statusSuccess: 'Success',
    statusRateLimit: 'Rate Limited',
    statusClientError: 'Client Error',
    statusServerError: 'Server Error',
  },
  profile: {
    changePassword: 'Change Password',
    oldPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm New Password',
    oldPasswordPlaceholder: 'Enter current password',
    newPasswordPlaceholder: 'At least 8 characters',
    confirmPasswordPlaceholder: 'Re-enter new password',
    oldPasswordRequired: 'Please enter current password',
    newPasswordRequired: 'Please enter new password',
    passwordMinLength: 'Password must be at least 8 characters',
    confirmPasswordRequired: 'Please confirm new password',
    passwordMismatch: 'Passwords do not match',
    passwordChangedSuccess: 'Password changed successfully',
    passwordChangeFail: 'Failed to change password',
  },
  notFound: {
    subtitle: 'Page not found',
    backHome: 'Back to Home',
  },
  strategy: {
    weighted_random: 'Weighted Random',
    round_robin: 'Round Robin',
    least_latency: 'Least Latency',
    least_cost: 'Least Cost',
    canary: 'Canary',
    least_busy: 'Least Busy',
  },
  logger: {
    title: 'System Logs',
    export: 'Export',
    clear: 'Clear',
    searchPlaceholder: 'Search keywords...',
    totalCount: '{0} entries',
    level: 'Level',
    time: 'Time',
    message: 'Message',
    empty: 'No logs',
    all: 'All',
    source: 'Source',
    errorStack: 'Error Stack',
    data: 'Data',
    context: 'Context',
  },
  license: {
    title: 'License Management',
    currentStatus: 'Current License Status',
    valid: 'License Valid',
    invalid: 'License Invalid',
    expired: 'License Expired',
    tier: 'License Tier',
    edition: 'Edition',
    status: 'Status',
    expiryDate: 'Expiry Date',
    noExpiry: 'Perpetual',
    fingerprint: 'Machine Fingerprint',
    cryptoMode: 'Crypto Mode',
    maxNodes: 'Max Nodes',
    licenseId: 'License ID',
    daysRemaining: '{0} days remaining',
    gracePeriod: 'Grace Period',
    communityNote:
      'Community edition requires no activation. Upgrade to Pro or Enterprise for more features.',
    upgradePrompt:
      'You are on Community edition. Import a license to upgrade to Pro or Enterprise.',

    activate: 'Online Activation',
    activateDesc: 'Enter your license key to activate online.',
    licenseKey: 'License Key',
    licenseKeyPlaceholder: 'Enter license key',
    licenseKeyRequired: 'Please enter a license key',
    activateButton: 'Activate',
    activating: 'Activating...',
    activateSuccess: 'License activated successfully',
    activateFail: 'Activation failed',

    import: 'Offline Import',
    importDesc: 'Upload a .lic file for offline activation.',
    selectFile: 'Select File',
    importButton: 'Import License',
    importing: 'Importing...',
    importSuccess: 'License imported successfully',
    importFail: 'Import failed',
    fileRequired: 'Please select a license file',
    fileHint: 'Only .lic files are supported',

    fetchFail: 'Failed to fetch license status',
    unexpectedError: 'Operation failed, please try again',
  },
  settings: {
    title: 'Settings',
    accountInfo: 'Account Information',
    username: 'Username',
    role: 'Role',
    version: 'System Version',
    frontend: 'Frontend',
    backend: 'Backend',
    tokenExpiry: 'Token Expiry (seconds)',
    licenseInfo: 'License Information',
    licenseTier: 'License Tier',
    licenseEdition: 'Edition',
    licenseStatus: 'License Status',
    licenseExpiry: 'License Expiry',
    connectionStatus: 'Connection Status',
    database: 'Database',
    cache: 'Cache (Redis)',
    featureSettings: 'Feature Settings',
    logContent: 'Record Request & Response Content',
    logContentDesc: 'When enabled, full request and response content will be recorded',
    updateSuccess: 'Settings updated successfully',
    updateFail: 'Failed to update settings',
    notAvailable: 'Requires backend update',
    viewLicenseDetail: 'View License Details',
  },
  pat: {
    title: 'Access Tokens',
    description: 'Personal access tokens (PAT) for scripts and automation tools',
    createToken: 'Create Token',
    totalTokens: '{n} tokens',
    tableName: 'Name',
    tableScopes: 'Scopes',
    tableStatus: 'Status',
    tableExpiresAt: 'Expires At',
    tableLastUsed: 'Last Used',
    tableCreatedAt: 'Created At',
    tableActions: 'Actions',
    statusActive: 'Active',
    statusRevoked: 'Revoked',
    statusExpired: 'Expired',
    neverUsed: 'Never used',
    expiresSoon: 'Expires in {n} days',
    moreScopes: '+{n}',
    revoke: 'Revoke',
    revokeConfirm: 'Revoke token "{name}"? Automation using it will fail immediately.',
    revokeSuccess: 'Token revoked',
    revokeFail: 'Failed to revoke token',
    nameLabel: 'Name',
    namePlaceholder: 'e.g. CI monitoring script',
    nameRequired: 'Name is required',
    scopesLabel: 'Scopes',
    scopesRequired: 'Select at least one scope',
    scopesDesc: 'The token can only call APIs matching the selected scopes',
    expiryNote: 'Fixed 90-day validity, not configurable',
    tokenCreated: 'Token Created',
    tokenWarning: 'Copy and save it now — it cannot be viewed again after closing',
    copyAndClose: 'Copy and Close',
    confirmedCopied: 'I have copied it',
    tokenCopied: 'Copied to clipboard',
    copyFailManual: 'Copy failed, please copy manually before closing',
    fetchFail: 'Failed to fetch tokens',
    errorScopeExceeded: 'Selected scopes exceed your account permissions',
  },
  errorBoundary: {
    title: 'Something went wrong',
    subtitle: 'Please try refreshing the page',
    retry: 'Retry',
  },
  org: {
    switcher: {
      searchPlaceholder: 'Search organizations...',
      backToPlatform: 'Back to Platform',
      platformOverview: 'Platform Overview',
      noResults: 'No organizations found',
      switchFailed: 'Failed to switch organization',
    },
  },
  globalDashboard: {
    title: 'Platform Overview',
    subtitle: 'Welcome back, {name}',
    statOrgs: 'Organizations',
    statMembers: 'Members',
    statKeys: 'API Keys',
    statRequests: 'Requests',
    statCost: 'Total Cost',
    statTokens: 'Token Usage',
    trendTitle: '7-Day Request Trend',
    orgOverview: 'Organization Overview',
    viewAll: 'View All',
    enterOrg: 'Enter',
    budgetUsage: 'Budget',
    healthHealthy: 'Healthy',
    healthWarning: 'Budget Alert',
    healthDanger: 'Critical',
    emptyTitle: 'No organizations yet',
    emptyDesc: 'Create your first organization to get started.',
    createFirst: 'Create Organization',
  },
  playground: {
    parameters: 'Parameters',
    model: 'Model',
    selectModel: 'Select model',
    selectModelFirst: 'Select a model first',
    systemPrompt: 'System Prompt',
    systemPromptPlaceholder: 'Enter system prompt...',
    maxTokens: 'Max Tokens',
    temperature: 'Temperature',
    topP: 'Top P',
    streaming: 'Streaming',
    send: 'Send',
    stop: 'Stop',
    clearChat: 'Clear Chat',
    inputPlaceholder: 'Type a message...',
    generating: 'Generating...',
    chatError: 'Chat request failed',
    streamError: 'Stream connection error',
    aborted: 'Generation stopped',
    tokensUsed: '{input} in / {output} out',
    latency: 'Latency: {ms}ms',
    emptyChat: 'Start a conversation',
    emptyChatDesc: 'Select a model and type a message below',
    noModels: 'No models available',
    noModelsDesc: 'Please configure models in the Models page first',
    roleUser: 'You',
    roleAssistant: 'Assistant',
    thinking: 'Thinking',
    thinkingFor: 'Thinking · {s}s',
    thoughtFor: 'Thought for {s}s',
    showThinking: 'Show thinking',
    hideThinking: 'Hide thinking',
    tabChat: 'Chat',
    tabImage: 'Image',
    imagePromptPlaceholder: 'Describe the image you want to generate...',
    generate: 'Generate',
    generatingImage: 'Generating image...',
    imageError: 'Image generation failed',
    imageSize: 'Size',
    imageQuality: 'Quality',
    imageQualityPlaceholder: 'Default',
    imageCount: 'Count',
    noImages: 'No images yet',
    noImagesDesc: 'Enter a prompt and click Generate',
    imageCost: 'Cost: ${cost}',
    download: 'Download',
    qualityStandard: 'Standard',
    qualityHd: 'HD',
    clearImages: 'Clear',
    roleSystem: 'System',
  },
  home: {
    eyebrow: 'Enterprise LLM API Gateway',
    title: 'A control plane between your apps and your models.',
    subtitle:
      'Smart routing · automatic failover · per-request observability · fine-grained key governance — every provider, converged into one controllable pipe behind a single API.',
    signIn: 'Sign in',
    cta: {
      primary: 'Enter console',
      github: 'GitHub',
    },
    cap: {
      routing: {
        title: 'Smart routing',
        desc: 'Six strategies: weighted / round-robin / least-latency / least-cost / canary / least-busy',
      },
      failover: {
        title: 'Automatic failover',
        desc: 'Silent rerouting on provider failure, with the full chain on the record',
      },
      observability: {
        title: 'Per-request observability',
        desc: 'TTFT, p99, token breakdown, fallback chains, cache hits — traceable per request',
      },
      keyGov: {
        title: 'Keys & quotas',
        desc: 'Model scoping + TPM/RPM + daily/weekly/monthly budget caps',
      },
    },
    footer: {
      license: 'License',
    },
    docs: {
      title: 'Get your API keys',
      subtitle: "Not sure where to grab a key? Jump straight to each provider's console and docs.",
      apiKey: 'API Key',
      docs: 'Docs',
      desc: {
        openai: 'GPT family and o1 reasoning models',
        anthropic: 'Claude family — strong long-context and coding',
        gemini: "Google's multimodal models",
        qwen: "Alibaba's Tongyi Qwen series",
        deepseek: 'Cost-efficient — V3 / R1 reasoning',
        glm: 'GLM-4 family from Zhipu AI',
        minimax: 'Multimodal — voice and long context',
        ernie: "Baidu's ERNIE series",
      },
    },
  },

  onboarding: {
    title: 'Setup Wizard',
    subtitle: 'Get connected and make your first call in just a few minutes.',

    // Welcome / mode selection
    welcomeTitle: 'Welcome to CrossLink',
    welcomeIntro:
      'Connect an upstream model and get a working API key in a few steps. Pick a mode to start:',
    modeQuick: 'Quick Setup',
    modeQuickDesc: 'Enter URL and key, auto-detect models. Recommended.',
    modeQuickBadge: 'Recommended',
    modeCustom: 'Custom Setup',
    modeCustomDesc: 'Manually pick adapter, models and key limits. Full control.',

    // Step titles
    stepAdapter: 'Source',
    stepCredentials: 'Credentials',
    stepModels: 'Models',
    stepKey: 'API Key',
    stepResult: 'Done',

    // adapter
    adapterTitle: 'Where do your models come from?',
    adapterDesc: 'Pick the upstream service type and we will connect with the right protocol.',
    adapterInferred: 'Detected from URL: {adapter}',
    needsUrl: 'Needs URL',
    needsKey: 'Needs key',

    // credentials
    baseUrl: 'API URL',
    baseUrlPlaceholder: 'https://api.example.com/v1',
    apiKey: 'API Key',
    apiKeyPlaceholder: 'sk-...',
    apiKeyHint: 'Used only for probing and creation. Never logged.',
    providerName: 'Provider name',
    providerNamePlaceholder: 'e.g. deepseek',
    providerNameHint: 'Lowercase identifier, cannot change after creation.',
    displayName: 'Display name',
    displayNamePlaceholder: 'e.g. DeepSeek',

    // probe
    probe: 'Test connection',
    probing: 'Testing connection...',
    probeOk: 'Connected (latency {ms} ms)',
    probeOkNoModels:
      'Connected, but this source does not support listing models — enter them manually.',
    probeFail: 'Connection failed',

    // models
    modelsFound: '{n} models available',
    selectModels: 'Select models to enable',
    defaultSelectedHint: 'The first chat model is selected for you.',
    selectAtLeastOne: 'Select at least one model.',
    catChat: 'Chat',
    catReason: 'Reasoning',
    catImage: 'Image',
    catAudio: 'Audio',
    catEmbed: 'Embedding',
    catOther: 'Other',
    manualMode: 'Enter models manually',
    manualHint:
      'This source cannot list models automatically. Type a model ID and press Enter, or pick a common one below.',
    manualPlaceholder: 'Model ID, e.g. gpt-4o',
    add: 'Add',
    presetTitle: 'Common models',
    modelCount: '{n} selected',

    // key
    keyName: 'Key name',
    keyNamePlaceholder: 'e.g. my-first-key',
    advanced: 'Advanced (optional)',
    tpmLabel: 'TPM limit',
    rpmLabel: 'RPM limit',
    budgetLabel: 'Budget cap',
    budgetPeriod: 'Budget period',
    optional: 'optional',

    // result
    resultTitle: 'Setup complete',
    resultIntro: 'Your upstream and key are ready. Here is a sample first request.',
    keyOnceWarning: 'This is the plain-text key — shown only once. Copy and save it now.',
    copyKey: 'Copy key',
    examplesTitle: 'Examples',
    curlTab: 'cURL',
    pythonTab: 'Python',
    nodeTab: 'Node.js',
    apiAddress: 'API URL',
    modelField: 'Model',
    gotoKeys: 'Go to Keys',
    gotoProviders: 'Go to Providers',
    testHint: 'Replace the URL and model with your actual values to run it.',

    // footer
    prev: 'Back',
    next: 'Next',
    skip: 'Skip',
    submit: 'Submit & finish',
    submitting: 'Submitting...',
    finish: 'Done',
    reopen: 'Re-run setup wizard',
    reopenFromEmpty: 'Run setup wizard',

    // errors
    errGlobalView: 'Switch into an organization before running the setup wizard.',
    errNoAdapter: 'Please choose a source type first.',
    errNoUrl: 'Please fill in the API URL first.',
    errNetwork: 'Network error — is the backend running?',
    errAuth: 'API key is invalid or expired. Please check and retry.',
    errForbidden: 'Access denied. Check the key permissions.',
    errTimeout: 'Connection timed out. Check the network or the API URL.',
    errConn: 'Cannot connect. Check the URL.',
    errUnknown: 'Probe failed. Please retry later.',
    errConflict: 'This provider name already exists. Rename and retry.',
    errUrlInvalid: 'Invalid API URL (must be http/https and not point to an internal address).',
    errInvalid: 'Some fields are invalid. Check the required items.',
    errSubmit: 'Submit failed. Please retry later.',
  },
}
